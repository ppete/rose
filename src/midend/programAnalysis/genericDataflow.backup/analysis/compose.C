...
/*class ArrayAccessAnalysis : ComposedAnalysis
{
   ...
   CPValueObject* Expr2Val(SgExpression* e, const Part& p) 
   {
...
class LiveDeadAnalysis : ComposedAnalysis
{
   ...
   LDMemLocObject* Expr2MemLoc(SgExpression* e, const Part& p) {
...
ChainComposer::ChainComposer(int argc, char** argv, list<ComposedAnalysis*>& analyses) : allAnalyses(analyses)
{
   ...
   allAnalyses.push_front(new SyntacticAnalysis());
...
boost::shared_ptr<RetObject> ChainComposer::callServerAnalysisFunc(SgNode* n, const Part& p, ComposedAnalysis* client, 
                                   FuncCaller<RetObject>& caller) {
  //Dbg::dbg << "ChainComposer::callServerAnalysisFunc() #doneAnalyses="<<doneAnalyses.size()<<endl;
  ...
  for(list<ComposedAnalysis*>::reverse_iterator a=doneAnalyses.rbegin(); a!=doneAnalyses.rend(); ) {
    ostringstream label; label << "Expr2Obj: " << (*a)->str("") << " ("<<n->class_name()<<" | "<<Dbg::escape(n->unparseToString())<<")";
    Dbg::enterFunc(label.str());
    ComposedAnalysis* curAnalysis = *a;
    doneAnalyses_back.push_back(curAnalysis);
    ...
    try {
      ...
      for(list<ComposedAnalysis*>::iterator a=doneAnalyses.begin(); a!=doneAnalyses.end(); a++)
        Dbg::dbg << "    "<<(*a)->str("        ")<<endl;
      Dbg::dbg << "Returning "<<v->str("")<<endl;
      
      Dbg::exitFunc(label.str());
      return v;
  ...
  cerr << "ERROR: no analysis implements method "<<caller.funcName()<<
           "(SgExpression)";
  ROSE_ASSERT(0);
}
...
class Expr2ValCaller : public FuncCaller<ValueObject>
{
  ...
  boost::shared_ptr<ValueObject> operator()(SgNode* n, const Part& p, ComposedAnalysis* a)
...
class Expr2MemLocCaller : public FuncCaller<MemLocObject>
{
  ...
  boost::shared_ptr<MemLocObject> operator()(SgNode* n, const Part& p, ComposedAnalysis* a)
...
class Expr2CodeLocCaller : public FuncCaller<CodeLocObject>
{
  ...
  boost::shared_ptr<CodeLocObject> operator()(SgNode* n, const Part& p, ComposedAnalysis* a)
  { return a->Expr2CodeLoc(n, p); }
  
  string funcName() { return "Expr2CodeLoc"; }
};

boost::shared_ptr<ValueObject> ChainComposer::Expr2Val(SgNode* n, const Part& p, ComposedAnalysis* client) { 
  Expr2ValCaller c;
  return callServerAnalysisFunc<ValueObject>(n, p, client, c);
}

boost::shared_ptr<MemLocObject> ChainComposer::Expr2MemLoc(SgNode* n, const Part& p, ComposedAnalysis* client) { 
  Expr2MemLocCaller c;
  return callServerAnalysisFunc<MemLocObject>(n, p, client, c);
}

boost::shared_ptr<CodeLocObject> ChainComposer::Expr2CodeLoc(SgNode* n, const Part& p, ComposedAnalysis* client) { 
  Expr2CodeLocCaller c;
  return callServerAnalysisFunc<CodeLocObject>(n, p, client, c);
}
