...
{*/
  public:
  typedef std::vector<PartPtr>      ConnectionContainer; // \todo rename to OutEdgeContainer
  ...
  bool propagateStateToNextNode(
              std::map<PartEdgePtr, LatticePtr >& curNodeState, PartPtr curDFNode,
              std::map<PartEdgePtr, LatticePtr >& nextNodeState, PartPtr nextDFNode);
  ...
  virtual std::map<PartEdgePtr, LatticePtr >& getLatticeAnte(NodeState *state) = 0;
  virtual std::map<PartEdgePtr, LatticePtr >& getLatticePost(NodeState *state) = 0;
  virtual void setLatticeAnte(NodeState *state, PartLattice dfInfo, bool overwrite) = 0;
  virtual void setLatticePost(NodeState *state, PartLattice dfInfo, bool overwrite) = 0;
...
class IntraFWDataflow  : public ComposedAnalysis
{
  ...
  std::map<PartEdgePtr, LatticePtr >& getLatticeAnte(NodeState *state);
  std::map<PartEdgePtr, LatticePtr >& getLatticePost(NodeState *state);
  void setLatticeAnte(NodeState *state, std::map<PartEdgePtr, LatticePtr >& dfInfo, bool overwrite);
  void setLatticePost(NodeState *state, std::map<PartEdgePtr, LatticePtr >& dfInfo, bool overwrite);
...
class IntraUndirDataflow  : public ComposedAnalysis
{
  public:
    ...
    static PartLattice emptyMap;

    PartLattice& getLatticeAnte(NodeState *state) { return emptyMap; }
    PartLattice& getLatticePost(NodeState *state) { return emptyMap; }
    void setLatticeAnte(NodeState *state, PartLattice& dfInfo, bool overwrite) { }
    void setLatticePost(NodeState *state, PartLattice& dfInfo, bool overwrite) { }

    //void transferFunctionCall(const Function &func, PartPtr callPart, CFGNode callCFG, NodeState *state) {};
    // was: \todo \filter PartPtr getUltimate(const Function &func) { return cfgUtils::getFuncStartCFG(func.get_definition(), filter); }
    ...
    vector<PartEdgePtr> getEdgesToDescendants(PartPtr part) { return vector<PartEdgePtr>(); }

    direction getDirection() { return none; }

    // Dummy transfer function since undirected analyses does not propagate flow information
    bool transfer(const Function& func, PartPtr p, CFGNode cn, NodeState& state, PartLattice& dfInfo) {
      return true;
    }
};
...
class printDataflowInfoPass : public IntraFWDataflow
{
  ...
  BoolAndLattice*        genLattice (const Function& func, PartPtr p, PartEdgePtr) /* override */;
  std::vector<NodeFact*> genFacts   (const Function& func, PartPtr p) /* override */;
  bool transfer(const Function& func, PartPtr p, NodeState& state, PartLattice& dfInfo) /* override */;
};
...
class MergeAllReturnStates : public UnstructuredPassIntraAnalysis
{
  ...
  public:
    ...
    : UnstructuredPassIntraAnalysis(analysis), _mergedLatsRetVal(), modified(false)
    {}

    MergeAllReturnStates(ComposedAnalysis* analysis, LatticePtr mLatsRetVal)
    : UnstructuredPassIntraAnalysis(analysis), _mergedLatsRetVal(mLatsRetVal->copy()), modified(false)
    {}
...
class SetAllReturnStates : public UnstructuredPassIntraAnalysis
{
  ...
  : UnstructuredPassIntraAnalysis(analysis), lats(static_cast<Lattice*>(0)), modified(false), paramList(0)
  {}

  SetAllReturnStates(ComposedAnalysis* analysis, LatticePtr _lats)
  : UnstructuredPassIntraAnalysis(analysis), lats(_lats), modified(false)
  {
    ROSE_ASSERT(lats.get());
  }
