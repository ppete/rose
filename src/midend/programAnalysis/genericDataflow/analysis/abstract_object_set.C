...
bool AbstractObjectSet::containsMust(const AbstractObjectPtr that) const
{
    ROSE_ASSERT(that);
    bool retval = false;
...
bool AbstractObjectSet::containsMay(const AbstractObjectPtr that) const
{
    ROSE_ASSERT(that);
    bool retval = false;
...
std::string AbstractObjectSet::strp(PartPtr part, std::string indent) const
{
    ostringstream oss;
...
AbstractObjectSet* AbstractObjectSet::copy() const
{
...
bool AbstractObjectSet::meetUpdate(const Lattice* thatL)
{
  try {
    ...
    if(mode == may) {
      ...
      for(std::list<AbstractObjectPtr>::const_iterator it=that.items.begin(); it!=that.items.end(); ++it) {
        // Do not copy over mappings with keys that are dead in this set's host part
...
bool AbstractObjectSet::operator==(const Lattice* thatL) const
{
  ...
  try {
    ...
    for(std::list<AbstractObjectPtr>::const_iterator it=items.begin(); it!=items.end(); ++it)
