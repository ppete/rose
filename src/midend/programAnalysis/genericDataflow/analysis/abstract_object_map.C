...
std::string AbstractObjectMap::strp(PartPtr part, std::string indent)  const
{
  ostringstream oss;
...
AbstractObjectMap* AbstractObjectMap::copy() const
{ return new AbstractObjectMap(*this); }

// overwrites the state of this Lattice with that of that Lattice
void AbstractObjectMap::copy(const Lattice* thatL) {
  try {
    const AbstractObjectMap& that = dynamic_cast <const AbstractObjectMap&>(*thatL);
    equalFunctor = that.equalFunctor;
    items = that.items;
  } catch (bad_cast & bc) {
    // \pp to use bad_cast, the dynamic_cast has to be to a reference type.
    //     bad casts to pointer types, return nullptr.
    ROSE_ASSERT(false);
  }
}
...
bool AbstractObjectMap::meetUpdate(const Lattice* thatL)
{
  typedef list<pair<list<MapElement>::iterator, pair<list<MapElement>::const_iterator, int> > > MustEqualContainer;
  ...
  try {
    ...
    MustEqualContainer thisMustEq2thatMustEq;
    ...
    for(list<MapElement>::const_iterator itThat=that.items.begin(); itThat!=that.items.end(); ++itThat)
    ...
    for(list<MapElement>::iterator itThis=items.begin();
      ...
      for(list<MapElement>::const_iterator itThat=that.items.begin();
    ...
    int thatIdx=0;
    list<MapElement>::const_iterator thatIt = that.items.begin();
    list<bool>::iterator thatMEIt=thatMustEq.begin();
    for(MustEqualContainer::iterator meIt=thisMustEq2thatMustEq.begin();
...
bool AbstractObjectMap::operator==(const Lattice* that) const
{
