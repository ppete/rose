...
public:
        ...
        bool copyFrom(const ConstrGraph &that, std::string indent="");
        ...
        // Returns true if this causes this constraint graph's state to change.
        bool copyFromReplace(const ConstrGraph &that, varID varTo, varID varFrom, std::string indent="");
...
protected:
        ...
        bool diffConstraints(const ConstrGraph &that, std::string indent="") const;

public:
        // Copies the constraints of cg into this constraint graph.
        // Returns true if this causes this constraint graph's state to change.
        bool copyConstraints(const ConstrGraph &that, std::string indent="");

        // Copies the constraints of cg associated with varFrom into this constraint graph,
        //    but mapping them instead to varTo.
        // Returns true if this causes this constraint graph's state to change.
        bool copyConstraintsReplace(const ConstrGraph &that, varID varTo, varID varFrom, std::string indent="");
...
public:
        ...
        // \pp already default implemented in the Lattice class
        Lattice* project(SgExpression* expr) const { return copy(); }
...
*/
        ...
        bool isEqZero(const varID& var, std::string indent="") const;

        // Returns true if v1*a = v2*b + c and false otherwise
        bool eqVars(const varID& v1, const varID& v2, int a=1, int b=1, int c=0, std::string indent="") const;
        bool eqVars(const varID& v1, const varID& v2, std::string indent="") const
        { return eqVars(v1, v2, 1, 1, 0, indent); }

        // If v1*a = v2*b + c, sets a, b and c appropriately and returns true.
        // Otherwise, returns false.
        bool isEqVars(const varID& v1, const varID& v2, int& a, int& b, int& c, std::string indent="") const;
        ...
        // Returns true if v1*a <= v2*b + c and false otherwise
        bool lteVars(const varID& v1, const varID& v2, int a=1, int b=1, int c=0, std::string indent="") const;

        // Returns true if v1*a < v2*b + c and false otherwise
        bool ltVars(const varID& v1, const varID& v2, int a=1, int b=1, int c=0, std::string indent="") const;
        ...
        // returns true if this causes this to change and false otherwise
        bool widenUpdate(const InfiniteLattice* that, std::string indent="");
        bool widenUpdate(const InfiniteLattice* that) { return widenUpdate(that, ""); }
        ...
        // returns true if this causes this to change and false otherwise
        bool widenUpdateLimitToThat(const InfiniteLattice* that, std::string indent="");

        // Common code for widenUpdate() and widenUpdateLimitToThat()
        bool widenUpdate_ex(const InfiniteLattice* that_arg, bool limitToThat, std::string indent="");
        ...
        bool meetUpdate(const Lattice* that, std::string indent="");
        bool meetUpdate(const Lattice* that) { return meetUpdate(that, ""); }
        ...
        bool meetUpdate_ex(const Lattice* that_arg, bool limitToThat, std::string indent="");
        ...
        bool OrAndWidenUpdate(const ConstrGraph* that, bool meet, bool OR, bool limitToThat, std::string indent="");
        ...
        void OrAndWidenUpdate_XinThatNotThis(
                                            ...
                                            std::map<varID, std::map<varID, affineInequality> >::const_iterator& itThatX,
        ...
        void OrAndWidenUpdate_YinThatNotThis(
                                            bool OR, bool limitToThat,
                                            std::map<varID, std::map<varID, affineInequality> >::const_iterator& itThatX,
                                            std::map<varID, affineInequality>::const_iterator& itThatY,
...
protected:
        ...
        bool checkSelfConsistency(std::string indent="") const
        {
          // \todo \pp remove this function
          return const_cast<ConstrGraph*>(this)->checkSelfConsistency(indent);
        }
...
public:
        ...
        const affineInequality* getVal(varID x, varID y, std::string indent="") const;
        ...
        // noConsistencyCheck - flag indicating whether this function should explicitly check the self-consisteny of this graph (=false)
        //                                                      or to not bother checking self-consistency and just return the last-known value (=true)
        std::pair<levels, constrTypes> getLevel(bool noConsistencyCheck=false, std::string indent="") const;
...
//protected:
        ...
        // If useIsBottom=true, isBottom() is used to determine whether the graph is =bottom.
        // Otherwise, the bottom variable is checked.
        std::string str(std::string indent, bool useIsBottom) const;

        // Returns a string that containts the representation of this constraint graph as a graph in the DOT language
        // that has the given name
        std::string toDOT(std::string graphName) const;

        // Returns a string that containts the representation of this constraint graph as a graph in the DOT language
        // that has the given name, focusing the graph on just the variables inside focusVars.
        std::string toDOT(std::string graphName, const std::set<varID>& focusVars) const;

public:
        /**** Comparison Functions ****/
        bool operator != (const ConstrGraph &that) const;
        bool operator == (const ConstrGraph &that) const ;
