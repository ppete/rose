...
class DeadPathElimPartEdge : public FiniteLattice, public PartEdge {
  ...
  DeadPathElimPartEdge* copy() const;
...
class DeadPathElimTransfer : public IntraDFTransferVisitor
{
  ...
  public:
    DeadPathElimTransfer(const Function &f, PartPtr part, CFGNode cn, NodeState& s, PartLattice& d,
                         DeadPathElimAnalysis* dpea);
...
class DeadPathElimAnalysis : public IntraFWDataflow
{
  ...
  DeadPathElimPartEdge*  genLattice(const Function& func, PartPtr part, PartEdgePtr pedge) /* override */;
