...
void DominatorLattice::copy(const Lattice* that_arg)
{
        const DominatorLattice* that = dynamic_cast<const DominatorLattice*>(that_arg);
        level = that->level;
        n = that->n;
        if(level == initialized)
                domNodes = that->domNodes;
...
bool DominatorLattice::copyFrom(const DominatorLattice* that, string indent)
{
        bool modified=false;
...
// We do not force child classes to define their own versions of this function since not all
//    Lattices have per-variable information.
void DominatorLattice::incorporateVars(const Lattice* that_arg)
{
        // Nothing to do. This analysis doesn't maintain per-variable information
        const DominatorLattice* that = dynamic_cast<const DominatorLattice*>(that_arg);
        copyFrom(that, "    ");
...
Lattice* DominatorLattice::project(SgExpression* expr) const
{
        // Nothing to do. This analysis doesn't maintain per-variable information
...
// returns true if this causes this to change and false otherwise
bool DominatorLattice::meetUpdate(const Lattice* that_arg)
{
        const DominatorLattice* that = dynamic_cast<const DominatorLattice*>(that_arg);
...
bool DominatorLattice::operator==(const Lattice* that_arg) const
{
        const DominatorLattice* that = dynamic_cast<const DominatorLattice*>(that_arg);
