...
bool isLiveMay(MemLocObjectPtr mem, LiveDeadMemAnalysis* ldma, PartEdgePtr pedge, string indent)
{
  ...
  if(pedge->source() && pedge->target()) {
    ...
    AbstractObjectSet* liveL = dynamic_cast<AbstractObjectSet*>(state->getLatticeAboveMod(ldma, pedge).get());
  ...
  // declaration so that clients can ask for its MemLocObject at edges coming into the declaration).
    ...
    PartLattice& e2lats = state->getLatticeAboveAllMod(ldma);
    ...
    for(PartLattice::iterator lats=e2lats.begin(); lats!=e2lats.end(); lats++) {
      ...
      AbstractObjectSet* liveL = dynamic_cast<AbstractObjectSet*>(lats->second.get());
    ...
    LatticePtr lats = state->getLatticeBelowMod(ldma);
    AbstractObjectSet* liveL = dynamic_cast<AbstractObjectSet*>(lats.get());
    ROSE_ASSERT(liveL);
