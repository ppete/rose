...
if(analysisDebugLevel>=1) Dbg::dbg << indent << "merged="<<merged<<endl;
                ...
                LatticePtr aboveCopy(pCFGState::getNodeState(func, n, this)->getLatticeAbove(this)->copy());
...
/*bool */ void pCFG_FWDataflow::propagateFWStateToNextNode(const Function& func,
                      ConstLatticePtr curNodeLattice, const pCFGNode& curNode, const NodeState& curNodeState,
                      LatticePtr nextNodeLattice, const pCFGNode& nextNode,
                      ConstrGraph* partitionCond, unsigned int pSet, string indent)
{
        ...
        if(analysisDebugLevel>=1)
        {
                    ...
                    itC!=curNodeLattices.end() && itN!=nextNodeLattices.end(); itC++, itN++, j++)
                {
                        ...
                        if(analysisDebugLevel>=1)
                        {
                                Dbg::dbg << indent << "        Current node below: Lattice: "<<curNodeLattice->str("            ")<<endl;
                                Dbg::dbg << indent << "        Next node above: Lattice : "<<nextNodeLattice->str("            ")<<endl;
                        }
                        /*Dbg::dbg << "-----\n"; fflush(stdout);
        ...
        ConstLatticePtr nextLatticeSource = curNodeLattice;

        if(analysisDebugLevel>=1)
                Dbg::dbg << indent << "partitionCond = "<<partitionCond<<endl;
        if(partitionCond)
        {
                LatticePtr curNodeLattices_withPartCond(curNodeLattice->copy());
                ...
                nextLatticeSource = curNodeLattices_withPartCond;
        }

        nextNodeLattice->copy(nextLatticeSource.get());
