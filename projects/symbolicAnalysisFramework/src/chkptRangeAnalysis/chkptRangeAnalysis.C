...
std::vector<NodeFact*>
ChkptRangeAnalysis::genFacts(const Function& func, const DataflowNode& n, const NodeState& state)
{
...
ConstrGraph*
ChkptRangeAnalysis::genLattice(const Function& func, const DataflowNode& n, const NodeState& state)
{
...
printf("ChkptRangeAnalysis::genInitState() state=%p\n", &state);*/
        ...
        // \pp \todo currently the ConstrGraph uses pointers to FiniteVarsExprsProductLattice*
        //           which we can likely replace with const FiniteVarsExprsProductLattice*
